...
if (-not (Test-Path "task_plan.md")) {
    $TemplatePlan = Join-Path $TemplateDir "task_plan.md"
    if (Test-Path $TemplatePlan) {
        Copy-Item $TemplatePlan "task_plan.md"
    } else {
        @"
...
if (-not (Test-Path "findings.md")) {
    $TemplateFindings = Join-Path $TemplateDir "findings.md"
    if (Test-Path $TemplateFindings) {
        Copy-Item $TemplateFindings "findings.md"
    } else {
        @"
# Findings & Decisions
...
if (-not (Test-Path "progress.md")) {
    $TemplateProgress = Join-Path $TemplateDir "progress.md"
    if (Test-Path $TemplateProgress) {
        Copy-Item $TemplateProgress "progress.md"
    } else {
        @"
# Progress Log
